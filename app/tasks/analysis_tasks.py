...
# --- Reusable Decorator for Async Celery Tasks ---
...
def launch_main_workflow(batch_task_id: int):
    """
    This task is triggered by the API. Its sole purpose is to build and
    launch the main complex workflow for a given batch task.
    """
    ...
    with Session(sync_engine) as db:
        logger.info(f"Launcher task received batch_id: {batch_task_id}. Building workflow...")
        
        sub_task_ids = db.exec(
            select(models.SubTask.id).where(models.SubTask.batch_task_id == batch_task_id)
        ).all()
        ...
        filtering_task_signature = run_first_filtering_task.s(batch_task_id)
        workflow = chain(analysis_group, filtering_task_signature)
        
        workflow.apply_async()
        logger.success(f"Successfully launched main workflow for batch_id: {batch_task_id}")
...
async def initial_analysis_task(sub_task_id: int):
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            properties = await zeo_client.get_all_properties(cif_content, cif_filename)
            sub_task.results = properties
